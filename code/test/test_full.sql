...
INSERT INTO dept (name, budget) VALUES ('Eng',1000),('Mkt',800),('Fin',1200)
    RETURNING id, name;
INSERT INTO emp (name, dept_id, salary, hdate, status, photo) VALUES
    ('Alice', 1,  900, '2020-01-15', 'active',   0x48656c6c6f),
    ('Bob',   1,  800, '2021-06-01', 'active',   NULL),
    ('Carol', 2,  700, '2019-11-20', 'inactive', NULL),
    ('Dave',  2,  600, '2022-03-10', 'active',   NULL),
    ('Eve',   3, 1200, '2015-05-01', 'active',   NULL),
    ('Frank', NULL, 500, '2023-07-01', 'active', NULL);
...
-- BLOB + TIMESTAMP 자동 삽입 확인
SELECT name, photo, joined_at FROM emp WHERE id = 1;
...
SELECT id, name, dept_id FROM emp WHERE id = dept_id;   -- col = col

-- ── 집계 ───────────────────────────────────────────────────────────────────
SELECT COUNT(*), SUM(amount), AVG(amount), MAX(amount), MIN(amount) FROM sal;
SELECT COUNT(DISTINCT dept_id) AS dept_cnt FROM emp;
...
SELECT e.name, d.name AS dept FROM dept d RIGHT JOIN emp  e ON e.dept_id = d.id ORDER BY e.id;
SELECT e.name, d.name AS dept FROM emp e FULL OUTER JOIN dept d ON e.dept_id = d.id ORDER BY e.name;
SELECT d.name AS dept_name, e.name AS emp_name FROM dept d CROSS JOIN emp e ORDER BY d.name, e.name LIMIT 6;
...
SELECT e.name, (SELECT s.amount FROM sal s WHERE s.eid = e.id) AS my_sal
...
SELECT eid  FROM sal WHERE grade = 'S5' UNION ALL SELECT eid FROM sal WHERE grade = 'S1';
SELECT name FROM emp WHERE dept_id IN (1,2) INTERSECT SELECT name FROM emp WHERE salary >= 700;
SELECT name FROM emp WHERE dept_id = 1 EXCEPT SELECT name FROM emp WHERE salary < 850;
SELECT eid  FROM sal WHERE amount >= 600 INTERSECT ALL SELECT eid FROM sal WHERE grade IN ('S3','S4','S5');
SELECT eid  FROM sal WHERE amount > 500 EXCEPT ALL SELECT eid FROM sal WHERE grade = 'S5';
...
FROM emp LIMIT 2;
SELECT salary/100 AS base, ROUND(salary/100,1), ABS(-999), CEIL(3.1), FLOOR(3.9),
       MOD(salary,7), 1+1 AS add_test FROM emp WHERE id <= 2;
SELECT name, hdate, joined_at, DATEDIFF('2026-05-01', hdate) AS days,
       DATE_ADD(hdate, INTERVAL 1 YEAR) AS nxt, DATE_FORMAT(hdate, '%Y-%m') AS ym
FROM emp WHERE status = 'active' LIMIT 2;
SELECT COALESCE(dept_id,-1) FROM emp WHERE dept_id IS NULL;
SELECT IFNULL(dept_id,0), NULLIF(dept_id,3) FROM emp LIMIT 2;
SELECT CAST('2026' AS INT), CAST('3.14' AS FLOAT), CAST(hdate AS TEXT) FROM emp LIMIT 1;

-- ── CASE / IF ──────────────────────────────────────────────────────────────
SELECT eid, amount,
    CASE WHEN amount >= 1000 THEN 'Exec' WHEN amount >= 800 THEN 'Senior' ELSE 'Junior' END AS lvl
FROM sal ORDER BY amount DESC;
SELECT name, IF(salary > 800, 'High', 'Normal') AS tier FROM emp ORDER BY salary DESC;
...
UPDATE emp SET email = CONCAT(name,'@co.com') WHERE status = 'active' RETURNING id, email;
...
-- ── UPDATE / DELETE RETURNING + FK CASCADE ─────────────────────────────────
UPDATE sal SET amount = amount * 2 WHERE grade = 'S1' RETURNING eid, amount;
UPDATE sal SET amount = amount / 2 WHERE grade = 'S1';
DELETE FROM emp WHERE id = 6 RETURNING name;
SELECT * FROM sal WHERE eid = 6;    -- 0 rows (ON DELETE CASCADE)

-- ── INSERT IGNORE / ON DUPLICATE KEY UPDATE ────────────────────────────────
INSERT IGNORE INTO dept (name) VALUES ('Eng'),('Legal');
...
UPDATE tags SET val = 'b' WHERE id = 1;
UPDATE tags SET val = 'zzz' WHERE id = 1;           -- ERROR: invalid ENUM
SELECT * FROM tags ORDER BY id;

-- ── TRANSACTION / SAVEPOINT / FOR UPDATE ───────────────────────────────────
SHOW LOCKS;
BEGIN;
SELECT id, salary FROM emp WHERE id = 1 FOR UPDATE;
SHOW LOCKS;
UPDATE emp SET salary = salary + 1 WHERE id = 1;
SAVEPOINT sp1;
UPDATE emp SET salary = 999 WHERE name = 'Alice';
...
ANALYZE TABLE emp;
EXPLAIN SELECT dept_id, salary FROM emp WHERE dept_id = 1;  -- Covering index
