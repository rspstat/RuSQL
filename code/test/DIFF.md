...
| 버퍼 풀 / 캐시 | InnoDB 버퍼 풀 (수 GB 단위) | shared_buffers | LRU 64 페이지 (인메모리) |
...
|------|-------|------------|--------|
| 정수 | TINYINT·SMALLINT·INT·BIGINT·UNSIGNED | SMALLINT·INT·BIGINT | INT |
...
| JSON | JSON (텍스트) | JSON·JSONB (바이너리 인덱스 가능) | ✗ |
| 배열 | ✗ | 모든 타입 배열 가능 | ✗ |
...
| VACUUM / 공간 회수 | 자동 Purge (InnoDB) | AUTOVACUUM | ✓ (수동 VACUUM 명령) |
...
| SELECT FOR UPDATE | ✓ | ✓ | ✓ |
| SELECT FOR SHARE | ✓ | ✓ | ✗ (FOR UPDATE만 가능) |
...
|------|-------|------------|--------|
| 문자열 | 50+ (CONCAT, SUBSTR, TRIM, REPLACE, LPAD, RPAD, REGEXP...) | 50+ (+ 정규식, FORMAT) | UPPER, LOWER, LENGTH, TRIM, CONCAT, SUBSTR, REPLACE, LPAD, RPAD |
| 날짜/시간 | 40+ (DATE_FORMAT, DATEDIFF, TIMESTAMPDIFF, PERIOD_ADD...) | 40+ (EXTRACT, AT TIME ZONE, AGE...) | NOW, CURDATE, DATE_FORMAT, DATE_ADD, DATEDIFF, YEAR, MONTH, DAY, HOUR, MINUTE, SECOND |
| 수학 | 20+ (ROUND, ABS, CEIL, FLOOR, MOD, SQRT, POWER, LOG, SIN...) | 20+ | ROUND, ABS, CEIL, FLOOR, MOD |
| 조건부 | COALESCE, IFNULL, NULLIF, IF, CASE | COALESCE, NULLIF, CASE, GREATEST, LEAST | COALESCE, IFNULL, NULLIF, IF, CAST |
| 정규식 | REGEXP, REGEXP_REPLACE (8.0+) | REGEXP_MATCH, REGEXP_REPLACE | ✗ |
| 타입 변환 | CAST, CONVERT | CAST, :: 연산자 | CAST |
| 사용자 정의 함수 | ✓ (CREATE FUNCTION) | ✓ (PL/pgSQL, PL/Python 등) | ✗ |
...
| 백업 / 덤프 | mysqldump, XtraBackup | pg_dump, pg_basebackup | ✗ |
| 저장 프로시저 | ✓ | ✓ (PL/pgSQL 등) | ✗ |
| 트리거 | ✓ | ✓ | ✗ |
| 이벤트 스케줄러 | ✓ | pg_cron 확장 | ✗ |
...
| 드라이버 호환 | JDBC, ODBC, Python, Go, Node.js... | JDBC, ODBC, libpq, Python, Go, Node.js... | rustdb-client CLI 전용 |
| 커넥션 풀 지원 | ✓ | ✓ | ✗ |
| Prepared Statements | ✓ | ✓ | ✗ |
| 배치 실행 | ✓ | ✓ | ✗ (멀티쿼리 ; 분리) |
...
| 항목 | 설명 | 난이도 |
|------|------|--------|
| 윈도우 함수 ROWS/RANGE 프레임 | `ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW` | 중 |
| INTERSECT / EXCEPT | 집합 연산 완성 | ✓ 완료 |
| NTILE / CUME_DIST / PERCENT_RANK | 윈도우 함수 나머지 | 소 |
| 집계 함수에 DISTINCT | `COUNT(DISTINCT col)`, `SUM(DISTINCT col)` | 소 |
| STDDEV / VARIANCE | 통계 집계 함수 | 소 |
| SHOW CREATE TABLE | 현재 스키마에서 DDL 역생성 | ✓ 완료 |
| MERGE / UPSERT 표준 문법 | SQL:2003 MERGE INTO | 중 |
| 정규식 함수 | REGEXP_MATCH, REGEXP_REPLACE | 중 |
...
| 버퍼 풀 확장 | 현재 64 페이지 고정 → 설정 가능, 더티 페이지 주기적 자동 flush | 소 |

#### 4순위 — 생태계

| 항목 | 설명 | 난이도 |
|------|------|--------|
| MySQL wire protocol 호환 | 기존 MySQL 드라이버(JDBC, Python mysql-connector 등)로 직접 접속 | 대 |
| 트리거 | BEFORE/AFTER INSERT/UPDATE/DELETE | 대 |
| 저장 프로시저 (기초) | BEGIN/END 블록, 변수, 조건문, 루프 | 대 |
| 뷰 업데이트 (Updatable View) | 단순 뷰에 INSERT/UPDATE/DELETE | 중 |
...
| CSV / SQL 익스포트 | 결과 테이블 → 파일 다운로드 | 소 |
| CSV 임포트 | 파일 선택 → INSERT 일괄 실행 | 소 |
| SHOW CREATE TABLE UI | 테이블 우클릭 → DDL 스크립트 보기 | 소 |
