...
impl DiskManager {
    ...
    /// "db.table" → ("db", "table").  No dot → ("rusql", key)
    ...
    /// table 인자는 "db.table" 또는 "table" (rusql 기본)
    ...
    pub fn load_schema(&self, table: &str) -> Option<TableSchema> {
        ...
        let path = format!("{}/{}.schema.json", self.table_dir(db), tbl);
        // 구버전 flat 경로: data/table.schema.json (rusql 테이블만)
        let flat_path = format!("{}/{}.schema.json", self.data_dir, tbl);
        let path = if Path::new(&path).exists() { path }
                   else if db == "rusql" && Path::new(&flat_path).exists() { flat_path }
                   else { return None; };
    ...
    /// 구버전 flat 파일은 "rusql.table" 형식으로 반환 (마이그레이션 지원).
        ...
        if let Ok(entries) = fs::read_dir(&self.data_dir) {
            for entry in entries.flatten() {
                ...
                if ftype.is_dir() {
                    ...
                    // 구버전 flat 파일 → "rusql.table"
                    let tbl = name.replace(".schema.json", "");
                    let qualified = format!("rusql.{}", tbl);
    ...
    pub fn load_views(&self, db: &str) -> HashMap<String, Statement> {
        ...
        let path = if Path::new(&path).exists() { path }
                   else if db == "rusql" && Path::new(&flat).exists() { flat }
