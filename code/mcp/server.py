...
app = FastAPI(title="RuSQL MCP Server", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def nl_to_sql(req: NLToSQLRequest):
    ...
    system = (
        f"You are a SQL expert for RuSQL (MySQL-compatible syntax). "
...
def explain_error(req: ErrorExplainRequest):
    ...
    prompt = f"""당신은 RuSQL(MySQL 호환 SQL 엔진)의 데이터베이스 전문가입니다.
...
def optimize(req: OptimizeRequest):
    ...
    schema_ctx = f"\n\n데이터베이스 스키마:\n{req.schema}" if req.schema.strip() else ""
    prompt = f"""당신은 RuSQL(MySQL 호환 SQL 엔진)의 쿼리 최적화 전문가입니다.
...
def schema_design(req: SchemaDesignRequest):
    ...
    prompt = f"""당신은 데이터베이스 설계 전문가입니다. RuSQL (MySQL 호환 문법)를 사용합니다.
...
def chat(req: ChatRequest):
    ...
    system = f"""You are an AI assistant for RuSQL, a MySQL-compatible SQL database engine.
