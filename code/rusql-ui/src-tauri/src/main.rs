...
fn handle_client(stream: TcpStream, shared: Arc<RwLock<SharedDatabase>>, log: Arc<Mutex<Vec<String>>>, sessions: Arc<Mutex<Vec<SessionInfo>>>) {
    ...
    let _ = writeln!(writer, "|   RuSQL Server v2.2.0                   |");
    let _ = writeln!(writer, "+-----------------------------------------+");
...
fn authenticate(user: String, password: String, data_dir: String, buffer_pool_size: usize, state: State<AppState>) -> bool {
    ...
    let new_exec = rusql_core::engine::executor::Executor::new_with_options(&data_dir, bp);
...
fn export_csv(query: String, file_path: String, state: State<AppState>) -> Result<String, String> {
    let mut exec = state.db.lock().unwrap();
    let result = {
        let mut p = rusql_core::parser::parser::Parser::new(&query);
        match p.parse() {
...
fn import_csv(table: String, file_path: String, state: State<AppState>) -> Result<String, String> {
    ...
    for line in lines {
        ...
        let mut p = rusql_core::parser::parser::Parser::new(&sql);
