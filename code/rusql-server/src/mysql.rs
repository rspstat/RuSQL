...
/// 연결마다 다른 20바이트 nonce 생성 (시각 + conn_id 기반 의사 난수).
/// null 바이트는 0x5a로 대체 (mysql_native_password 프로토콜 요구사항).
fn make_nonce(conn_id: u32) -> [u8; 20] {
    use std::time::{SystemTime, UNIX_EPOCH};
    let t = SystemTime::now()
        .duration_since(UNIX_EPOCH).unwrap_or_default()
        .as_nanos() as u64;
    let mut n = [0u8; 20];
    for i in 0..8  { n[i]     = ((t        >> (i * 8)) & 0xff) as u8; }
    for i in 0..4  { n[8 + i] = ((conn_id  >> (i * 8)) & 0xff) as u8; }
    // 뒷 8바이트: t의 상위 비트 + 고정 salt
    for i in 0..4  { n[12 + i] = (((t >> 32) >> (i * 8)) & 0xff) as u8; }
    n[16] = 0x52; n[17] = 0x44; n[18] = 0x42; n[19] = 0x21; // "RDB!"
    for b in n.iter_mut() { if *b == 0 { *b = 0x5a; } }
    n
}

fn handshake_pkt(conn_id: u32, nonce: &[u8; 20]) -> Vec<u8> {
    let mut p = Vec::new();
    p.push(10u8);                                           // protocol version 10
    nulstr(&mut p, "5.7.0-rusql");                        // server version
...
/// executor 출력을 파싱해 (컬럼명, 행 목록)을 반환. 테이블 형식이 아니면 None.
/// 두 가지 형식을 지원:
///   1. 박스 형식 (+---+|val|) — SHOW LOCKS/WAL 등
///   2. 탭 구분 형식 (col\tcol\nval\tval) — SELECT / SHOW TABLES/DATABASES / DESCRIBE 등
fn parse_table(out: &str) -> Option<(Vec<String>, Vec<Vec<String>>)> {
    ...
    if trimmed.starts_with('+') {
        ...
        return if cols.is_empty() { None } else { Some((cols, rows)) };
    }
    ...
    // 탭이 없고 한 줄짜리 → 메시지 (예: "Table created", "Query OK" 등)
    if cols.len() == 1 && lines.len() == 1 {
        // 헤더처럼 생긴 단어인지 확인 (식별자 패턴: 알파벳/밑줄로 시작, 공백 없음)
        let w = cols[0].as_str();
        let looks_like_header = !w.is_empty()
            && w.chars().next().map(|c| c.is_alphabetic() || c == '_').unwrap_or(false)
            && !w.contains(' ')
            && !w.chars().any(|c| c == '(' || c == ')' || c == '\'');
        if !looks_like_header { return None; }
        // 헤더만 있고 데이터 없는 빈 결과 (SHOW DATABASES 등 결과 없을 때)
        return Some((cols, vec![]));
    }
    ...
    let rows: Vec<Vec<String>> = lines[1..].iter()
        .map(|l| {
            let mut cells: Vec<String> = l.split('\t')
                .map(|s| s.trim().to_string())
                .collect();
...
fn mysql_compat(q: &str, exec: &mut Executor) -> Option<Result<String, String>> {
    ...
    {
        return Some(Ok(box_table(&["version"], &[vec!["5.7.0-rusql".to_string()]])));
    }
    ...
    if up.starts_with("SHOW ENGINES") {
        let rows = vec![vec![
            "RuSQL".to_string(), "DEFAULT".to_string(),
            "RuSQL B+Tree Storage Engine".to_string(),
...
pub fn handle_mysql_client(mut stream: TcpStream, shared: Arc<RwLock<SharedDatabase>>, conn_id: u32) {
    ...
    // 1. Server sends Handshake (연결마다 다른 nonce)
    let nonce = make_nonce(conn_id);
    if write_packet(&mut stream, 0, &handshake_pkt(conn_id, &nonce)).is_err() { return; }
    ...
    let username = String::from_utf8_lossy(&resp[pos..pos + uend]).to_string();
    pos += uend + 1;

    // auth-response (mysql_native_password: 20바이트 챌린지 응답)
    let auth_response: Vec<u8> = if client_caps & 0x8000 != 0 {
        // CLIENT_SECURE_CONNECTION: 1-byte length prefix
        let auth_len = resp.get(pos).copied().unwrap_or(0) as usize;
        pos += 1;
        let r = resp.get(pos..pos + auth_len).unwrap_or(&[]).to_vec();
        pos += auth_len;
        r
    } else {
        // null-terminated (구형 프로토콜)
        let end = resp[pos..].iter().position(|&b| b == 0).unwrap_or(resp.len() - pos);
        let r = resp.get(pos..pos + end).unwrap_or(&[]).to_vec();
        pos += end + 1;
        r
    };
    ...
    let init_db = if client_caps & 0x0008 != 0 && pos < resp.len() {
        let end = resp[pos..].iter().position(|&b| b == 0).unwrap_or(resp.len() - pos);
        String::from_utf8_lossy(&resp[pos..pos + end]).to_string()
    } else {
        String::new()
    };

    // 3. 인증 검증
    let auth_ok = shared.read().unwrap()
        .verify_mysql_native_password(&username, &nonce, &auth_response);
    if !auth_ok {
        let msg = format!(
            "Access denied for user '{}' (using password: {})",
            username,
            if auth_response.is_empty() { "NO" } else { "YES" }
        );
        let _ = write_packet(&mut stream, 2, &err_pkt(&msg));
        return;
    }
