...
|---|---|---|---|---|
| P0 | VACUUM이 PK 인덱스를 손상 | `executor.rs:7679,8105` — VACUUM/AUTO VACUUM이 `row.values().next()`로 PK값을 가져옴(HashMap 순서 무작위) | PK 조회·O(1) DELETE·중복검사가 항상 정확 | 낮음 |
| P0 | WHERE 우변 산술식 미지원→오답 | `parser.rs:900-984` — `WHERE v > id + 100`에서 `+100`이 버려져 `v > id`로 잘못 평가(실증 확인) | WHERE 절 결과 신뢰성 확보 | 중간 |
| P0 | 파서가 문장 끝 잔여 토큰 미검증 | `parser.rs:338` — 전체 토큰 소비 확인 안 해 `...))) garbage`도 조용히 실행(실증 확인) | 잘못된 SQL을 명확한 에러로 표시 | 낮음 |
| P0 | B+Tree가 숫자형 문자열 키를 충돌 | `btree.rs:34-38` — "007"과 "07"이 둘 다 7.0으로 파싱돼 동일 키 취급(실증: 삽입 오류) | 우편번호·SKU 등 문자열 PK 안전 지원 | 중간 |
| P0 | 복합 인덱스가 숫자를 텍스트로 정렬 | `composite_index.rs` — `\x00` 결합 키라 숫자 인식 실패, "10"<"9"로 정렬 | 복합 인덱스 범위/정렬 쿼리 정확도 | 중간 |
| P0 | 쿼리 캐시가 비결정 함수 영구 캐싱 | `executor.rs:686-759` — NOW()/RAND()/UUID() 포함 SELECT가 무효화 없이 과거값 반환 | 시간·난수 쿼리 정확성 | 낮음 |
| P0 | 이중 단항 마이너스 파싱 오류 | `parser.rs:1039-1051` — `SELECT - -5`가 "--5" 문자열 생성(실증 확인) | 산술식 파싱 신뢰성 | 낮음 |
| P0 | RENAME COLUMN 후 인덱스 무효화 | `executor.rs:5863-5882` — 인덱스 메타 미갱신, 에러 없이 SeqScan 전락 | 스키마 변경 후 인덱스 성능 유지 | 중간 |
...
| P1 | BEGIN...END 내부 세미콜론으로 프로시저 조기 실행 | `rusql-server/main.rs:94-111,241-247` + UI 서버 동일 로직 중복 | 저장 프로시저/트리거 생성 안정성 | 중간 |
...
| P2 | TLS/SSL 전무 | 레포 전체에 rustls/native-tls 없음 | 전송 계층 보안 | 높음 |
