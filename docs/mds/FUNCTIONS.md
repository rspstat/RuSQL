...
- [x] CSV 익스포트 — 결과 패널의 "⬇ CSV" 버튼, 순수 클라이언트 사이드(Blob 생성 + `<a download>`)로 SELECT 결과를 CSV 파일로 저장. (한 번도 호출되지 않던 백엔드 `export_csv` Tauri 커맨드는 2026-09-14 죽은 코드 정리에서 제거됨 — `PLAN.md` Section G 참고)
...
- [x] **Server Manager AI MCP 패널** — "AI MCP" 우측 버튼으로 슬라이드 패널 토글; "Claude Desktop 자동 연결" 버튼 → `setup_mcp_config` Tauri 커맨드로 `%AppData%\Claude\` (일반 설치) + `LocalCache\Roaming\Claude\` (Windows Store) 두 경로에 BOM 없는 UTF-8로 `claude_desktop_config.json` 자동 생성·병합, **25개 도구** 중 파괴적이거나 재확인이 필요한 것(삭제/자격증명 변경/위험 SQL 확인 우회/서버 중지)을 뺀 나머지만 `alwaysAllow` 자동 등록 (Claude Desktop 자체 팝업이 마지막 방어선); `where python`으로 `mcp` 패키지 설치된 Python 자동 탐지; `RUSQL_APP_PATH` env로 이 커맨드를 실행 중인 프로세스 자신의 경로(`std::env::current_exe()`)도 함께 전달해 `launch_app` 도구가 앱이 안 켜져 있을 때 무엇을 실행할지 알 수 있게 함. `list_connections`/`add_connection`/`update_connection`/`delete_connection` 4개는 홈 화면의 저장된 연결 목록(`code/data/connections.json`)을 UI와 그대로 공유 — UI가 홈 화면에서 3초마다 이 파일을 다시 읽으므로, Claude가 추가/삭제/수정하면 앱 재시작 없이 반영됨(비밀번호는 `list_connections` 응답에서 항상 제외)
- [x] **MCP 쿼리 에디터 UI 제어** (`write_to_editor`/`new_editor_tab`/`close_editor_tab`/`switch_editor_tab`/`list_editor_tabs`/`get_editor_tab_content`/`execute_in_editor`, 7개 도구) — Phase 17에서 완전히 죽어있던 프로토콜(`_run_ui`가 SQL TCP 포트로 `UI:{...}` 문자열을 보내지만 서버 쪽이 아예 구현 안 됨)이 제거됐던 자리에, 실제로 응답하는 쪽이 있는 새 브리지로 재구현. `code/data/ui_commands.json` 큐에 MCP가 `{id, action, params, target_instance}`를 pending으로 넣으면 → main.rs의 배경 스레드(400ms 폴링, 이 인스턴스 자신의 id와 `target_instance`가 다르면 무시)가 집어 `in_progress`로 바꾸고 Tauri `ui-command` 이벤트로 프런트에 전달 → App.tsx의 `uiCmdHandlerRef`(이미 존재했지만 아무도 이벤트를 emit하지 않아 죽어있던 디스패처)가 실제로 탭 생성/전환/닫기, Monaco 에디터에 SQL 입력(undo 히스토리 보존), 화면에서 실제 쿼리 실행까지 수행 후 `complete_ui_command`로 결과를 채워 `done`으로 반환 → MCP가 그 결과를 폴링해 돌려줌(기본 30초, `execute_in_editor`는 느린 쿼리를 위해 `timeout_seconds`로 최대 600초까지 조정 가능; 로그인 화면이면 `login` 액션 외엔 전부 거부됨). 이 큐 파일은 Rust 배경 스레드/Python 양쪽이 빈번히 동시 쓰기 때문에 `code/data/ui_commands.json.lock`(O_CREAT|O_EXCL 방식) 상호 배제 락으로 read-modify-write를 직렬화 — 락 없이 테스트했을 때 동시 도구 호출 2개 중 하나가 조용히 사라지는 경합을 실제로 재현·확인 후 추가; 임시 파일명도 호출마다 고유하게(Windows `ERROR_SHARING_VIOLATION`/경합 방지). 실제 실행 중인 앱을 대상으로 8개 시나리오(탭 목록 조회/쿼리 실행/탭 생성/에디터 쓰기/탭 내용 조회/탭 전환/탭 닫기) 전부 라이브 검증
- [x] **MCP 위험한 SQL 안전장치** (`confirm_dangerous_sql`) — `execute_sql`/`execute_in_editor`가 SQL을 실행하기 전에 정규식 휴리스틱(`DROP`/`TRUNCATE`, 또는 `WHERE` 없는 `UPDATE`/`DELETE`)으로 위험 여부를 먼저 판단해 감지되면 실행하지 않고 거부 메시지만 반환 — Claude가 사용자에게 명시적으로 확인받은 뒤에만 `confirm_dangerous_sql`을 별도로 호출해야 실제로 실행됨(같은 이름으로 재호출해야 하므로 Claude가 판단만으로 우회 불가). `execute_in_editor`는 이미 탭에 있는 SQL(사용자가 직접 입력한 것)을 그냥 실행하는 경우는 검사 대상에서 제외 — Claude가 새로 주입하는 SQL만 검사. `via_editor=True`로 UI에서 보이게 실행할지, 엔진에 바로(안 보이게) 실행할지 선택 가능
- [x] **MCP 다중 앱 인스턴스 지원** (`list_app_instances`) — RuSQL 창을 여러 개 띄워 서로 다른 DB에 동시 접속해 쓰는 경우를 위해, 각 인스턴스가 `code/data/app_instances.json`에 자기 존재(`id`/`pid`/로그인 여부/현재 DB)를 배경 스레드로 ~2초마다 기록(하트비트가 15초 이상 끊기면 다음 기록 때 정리 — 창이 비정상 종료돼도 죽은 항목으로 안 남음). 위의 모든 UI 제어 도구 + `login`/`start_server`/`stop_server`/`get_server_status`가 `instance` 파라미터로 특정 창을 지정 가능(생략하면 기존처럼 아무 인스턴스나 먼저 집어가는 쪽이 처리)
- [x] **MCP 앱 실행/로그인** (`launch_app`, `login`) — `launch_app`은 실행 중인 인스턴스가 없을 때만 `RUSQL_APP_PATH`(설정 시점의 앱 자신의 경로)로 새 프로세스를 띄움. `login`은 홈 화면(로그인 전)에 떠있는 창을 저장된 Connection(id 또는 고유 이름)으로 로그인시킴 — 기존엔 `"ui-command"` 이벤트 리스너 자체가 로그인 후에만 등록돼 로그인 전 명령을 아예 못 받았는데, 리스너를 앱 시작 시부터 항상 등록하도록 바꾸고 로그인 게이트를 `login` 액션만 예외로 두는 액션별 분기로 옮겨서 해결. 비밀번호가 저장 안 된 연결이거나 이미 로그인된 창이면 거부
- [x] **MCP 서버 제어** (`start_server`/`stop_server`/`get_server_status`) — Server Manager 탭의 공개 리스너(네이티브 + 선택적 MySQL 와이어 프로토콜)를 원격으로 제어. 로그인된 세션의 `connId`를 그대로 재사용하므로 로그인 상태가 전제(로그인 시 자동으로 붙는 엔진 자체와는 별개 — 그건 이미 항상 켜져 있음)
