...
| AI 연동 | △ (HeatWave AutoML) | ✗ | △ (Oracle AI) | ✓ (AI MCP — mcp_server.py: Claude Desktop 연동, stdio JSON-RPC, **25개 도구** — execute_sql · confirm_dangerous_sql(execute_sql/execute_in_editor가 DROP·TRUNCATE·WHERE 없는 UPDATE/DELETE를 자동 거부하고, 사용자 확인 후에만 이 도구로 재호출해야 실제 실행됨) · list_databases · list_tables · get_table_schema · explain_query · get_indexes · sample_data · list_connections · add_connection · update_connection · delete_connection(이 4개는 `code/data/connections.json` 파일을 UI와 공유) · write_to_editor · new_editor_tab · close_editor_tab · switch_editor_tab · list_editor_tabs · get_editor_tab_content · execute_in_editor(이 7개는 `code/data/ui_commands.json` 큐 + Tauri `ui-command` 이벤트로 실제 쿼리 에디터를 조작 — 탭 생성/전환/닫기, 에디터에 SQL 입력, 화면에서 실제로 쿼리 실행까지 가능; execute_in_editor는 느린 쿼리를 위한 timeout_seconds 파라미터 지원) · list_app_instances · launch_app · login · start_server · stop_server · get_server_status(RuSQL 창을 여러 개 띄워도 `code/data/app_instances.json` 하트비트 레지스트리로 서로 구분해 `instance` 파라미터로 특정 창을 지정 가능; launch_app은 앱이 안 켜져 있으면 대신 실행, login은 홈 화면에서 저장된 연결로 로그인, start_server/stop_server/get_server_status는 Server Manager의 공개 리스너를 제어), SELECT 결과 JSON 배열 구조화, alwaysAllow 자동 설정(파괴적 도구는 제외, Claude Desktop 자체 팝업이 마지막 방어선), API 키 불필요, UI 자동 연결 버튼) |
