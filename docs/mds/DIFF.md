...
| AI 연동 | △ (HeatWave AutoML) | ✗ | △ (Oracle AI) | ✓ (AI MCP — mcp_server.py: Claude Desktop 연동, stdio JSON-RPC, **17개 도구** — execute_sql · list_databases · list_tables · get_table_schema · explain_query · get_indexes · sample_data · list_connections · add_connection · delete_connection(이 3개는 `code/data/connections.json` 파일을 UI와 공유해 Claude가 홈 화면의 저장된 연결을 추가/삭제 가능) · write_to_editor · new_editor_tab · close_editor_tab · switch_editor_tab · list_editor_tabs · get_editor_tab_content · execute_in_editor(이 7개는 `code/data/ui_commands.json` 큐 + Tauri `ui-command` 이벤트로 실제 쿼리 에디터를 조작 — 탭 생성/전환/닫기, 에디터에 SQL 입력, 화면에서 실제로 쿼리 실행까지 가능), SELECT 결과 JSON 배열 구조화, alwaysAllow 자동 설정 (허용 팝업 없음), API 키 불필요, UI 자동 연결 버튼) |
