...
| AI 연동 | △ (HeatWave AutoML) | ✗ | △ (Oracle AI) | ✓ (AI MCP — mcp_server.py: Claude Desktop 연동, stdio JSON-RPC, **10개 도구** — execute_sql · list_databases · list_tables · get_table_schema · explain_query · get_indexes · sample_data · list_connections · add_connection · delete_connection(뒤 3개는 `code/data/connections.json` 파일을 UI와 공유해 Claude가 홈 화면의 저장된 연결을 추가/삭제 가능), SELECT 결과 JSON 배열 구조화, alwaysAllow 자동 설정 (허용 팝업 없음), API 키 불필요, UI 자동 연결 버튼) |
